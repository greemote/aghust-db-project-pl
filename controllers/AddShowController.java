...
public class AddShowController
{
    ...
    public void initialize()
    {
        try
        {
            PreparedStatement pst = ConnectionVariable.c.prepareStatement("SELECT miejscowosc FROM BD_projekt.kino WHERE id_kino = ?", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            pst.setInt(1, idCinema);
            ResultSet rs = pst.executeQuery();
            rs.next();
            city.setText(rs.getString(1));
            ...
            rs.next();
            count = rs.getInt(1);
            pst = ConnectionVariable.c.prepareStatement("SELECT tytul FROM BD_projekt.film", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            rs = pst.executeQuery();
            for(int i = 1; i <= count; ++i)
            {
                rs.next();
                chooseMovie.getItems().add("[" + i + "] " + rs.getString(1));
            }

            pst = ConnectionVariable.c.prepareStatement("SELECT COUNT(*) FROM BD_projekt.sala WHERE id_kino = ?", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            pst.setInt(1, idCinema);
            rs = pst.executeQuery();
            rs.next();
            count = rs.getInt(1);
            ...
            pst = ConnectionVariable.c.prepareStatement("SELECT COUNT(*) FROM BD_projekt.technologia", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            rs = pst.executeQuery();
            rs.next();
            count = rs.getInt(1);
            pst = ConnectionVariable.c.prepareStatement("SELECT nazwa FROM BD_projekt.technologia", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            rs = pst.executeQuery();
            for(int i = 1; i <= count; ++i)
            {
                rs.next();
                chooseTechnology.getItems().add("[" + i + "] " + rs.getString(1));
            }

            pst = ConnectionVariable.c.prepareStatement("SELECT COUNT(*) FROM BD_projekt.tlumaczenie", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            rs = pst.executeQuery();
            rs.next();
            count = rs.getInt(1);
            pst = ConnectionVariable.c.prepareStatement("SELECT nazwa FROM BD_projekt.tlumaczenie", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            rs = pst.executeQuery();
            for(int i = 1; i <= count; ++i)
            {
                rs.next();
                chooseTranslation.getItems().add("[" + i + "] " + rs.getString(1));
            }
